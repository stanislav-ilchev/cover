/*   anneal_fast.c
**
**   Optimized simulated annealing with faster PRNG and sorting.
**   Drop-in replacement for anneal.c with same interface.
**
*/
...
/*
** Static variables for neighbor computation
*/
...
costType initSolution(void)
{
  ...
  /* Initialize fast PRNG - use time-based seed if PRNGseed not set */
  {
    unsigned long long seed = PRNGseed;
    if (seed == 0) seed = (unsigned long long)time(NULL);
    seed ^= (unsigned long long)((size_t)&seed); /* add some entropy */
    fast_seed(seed);
  }
  ...
  for(i = 0; i < b; i++) {
	...
	if(useMask)
	  calculateOneCoveringMaskFast(ksetMask[i], coveringsPtr);
	else
	  calculateOneCovering(kset[i], coveringsPtr);
      }
      else {
	if(useMask)
	  calculateOneCoveringMaskFast(ksetMask[i], coverings);
	else
...
static rankType randomNeighbor(rankType curr)
{
  ...
  subset[fast_rnd(k)] = csubset[fast_rnd(v-k)];
...
** Optimized version with fast bit selection.
*/

static inline rankType randomNeighborMask(maskType currMask, maskType *outMask)
{
  maskType comp;
  int removeBit, addBit;

  comp = cachedFullMask ^ currMask;
  removeBit = fast_select_bit(currMask, fast_rnd(k));
  addBit = fast_select_bit(comp, fast_rnd(v - k));
...
/*
** calculateOneCoveringMaskFast() - optimized version using insertion sort
** for small arrays instead of qsort.
*/
...
void calculateOneCoveringMaskFast(maskType mask, rankType *buf)
{
  ...
  *coverptr = binCoef[v][m]; /* sentinel */
  ...
  /* Use fast insertion sort for typical small arrays */
  fast_sort_ranks_hybrid(buf, coverLen - 1);
}


/*
** computeNeighbor() calculates the cost difference between the current
** solution and a random neighbor of the current solution.
** Optimized with fast PRNG and inlined bit operations.
*/
...
costType computeNeighbor(void)
{
  ...
  if(onTheFly) {
    ...
    if(onTheFlyCache) {
      ...
      nextPtr = coverings;
      if(useMask)
	calculateOneCoveringMaskFast(nextMask, nextPtr);
      else
    ...
    nextS = neighbors[currS * neighborLen + fast_rnd(neighborLen)];
    currPtr = coverings + currS * coverLen;
    nextPtr = coverings + nextS * coverLen;
  }

  /* Merge-diff loop - already optimal */
  for(;;) {
    if(*currPtr == *nextPtr) {
...
/*
** approxInitT() tries to find a good initial value for T.
*/
...
/*
** simulatedAnnealing() is the algorithm.
** Optimized with fast PRNG for accept/reject decisions.
*/
			    ...
			    int iterLength, int frozen, int endLimit)
{
  ...
  if(verbose)
    printf("Starting annealing (fast)...\n\n");
  calculateCosts();
  ...
  if(verbose) {
    ...
    for(i = 0; i < T_PRINT_ITER; i++) {
      ...
      if(costDelta > 0) {
	m1++;
	if(fast_random01() < ExpProb(costDelta / T))
	  m2++;
      }
    }
    if(m1 == 0) {
			    ...
			    int iterLength, int frozen, int endLimit)
{
  ...
  if(verbose)
    printf("Starting threshold accepting (fast)...\n\n");
  calculateCosts();
  currCost = initSolution();
  bestSeen = lastCost = currCost;
...
costType recordToRecordTravel(int threshold, int frozen, int endLimit)
{
  ...
  costType acceptLimit;
  int i, m1, m2;
  int itersSinceImprovement = 0;

  if(verbose)
    printf("Starting Record-to-Record Travel (fast, threshold=%d)...\n\n", threshold);
  calculateCosts();
  currCost = initSolution();
  bestSeen = currCost;
...
/*
** `localOptimization()' performs hill-climbing.
*/
...
costType localOptimization(int frozen, int endLimit)
{
  ...
  if(verbose >= 2)
    printf("Starting local optimization (fast)...\n");
  calculateCosts();
  currCost = initSolution();
  do {
    while(notChanged < frozen) {
