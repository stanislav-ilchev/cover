...
float coolFact=1, initProb=0.5;
int v=27, k=6, t=3, m=4, b=86;
int testCount = 1;
int restrictedNeighbors = 0;
float initialT = 0.6;
int frozen = 1000000000;
int endLimit = 250;
int apprexp = 0;
int Tset = 0;
int L = 24;
...
{
#ifdef _WIN32
  /* getrusage is not available on Windows */
...
void printParams(FILE *fp)
{
  ...
  fprintf(fp, "EndLimit      = %d\nlocal         = %d\nTA            = %d\nRR            = %d\nThreshold     = %d\napprexp       = %d\n"
	  ...
	  "log           = %s\nresult        = %s\nSolX          = %d\n"
	  "verbose       = %d\nMemoryLimit   = %lu\n"
	  "check         = %d\n\n", endLimit, localOpt, thresholdAccept, recordToRecord, fixedThreshold, apprexp, onTheFly,
...
int main(int argc, char **argv)
{
  ...
  do {
    for(count = 0; count < testCount; count++) {
      iterCounter = 0;
      cpuStart = cpu_time_seconds();
      if(localOpt)
	finalCost = localOptimization(L, endLimit);
      else if(recordToRecord)
	finalCost = recordToRecordTravel(fixedThreshold, frozen, endLimit);
      else if(thresholdAccept)
	finalCost = thresholdAccepting(initialT, coolFact, L, frozen, endLimit);
      else
	...
	bestCost = finalCost;
      cpuEnd = cpu_time_seconds();
      CPU = (float) (cpuEnd - cpuStart);
      costSum += finalCost;
