...
} vars[] =
{
  ...
  {"EL", "%d", &endLimit},
  {"local", "%d", &localOpt},
  {"TA", "%d", &thresholdAccept},
  {"ThresholdAccept", "%d", &thresholdAccept},
  {"RR", "%d", &recordToRecord},
  {"RecordToRecord", "%d", &recordToRecord},
  {"TH", "%d", &fixedThreshold},
  {"Threshold", "%d", &fixedThreshold},
  {"apprexp", "%d", &apprexp},
  {"OntheFly", "%d", &onTheFly},
  {"OF", "%d", &onTheFly},
  {"Pack", "%d", &pack},
...
void parseArguments(int argc, char **argv)
{
  ...
  for(i = 1; i < argc; i++) {
    ...
    stripQuotes(value);
    found = 0;
    if(!strcmp(name, "start") || !strcmp(name, "Start")) {
      strncpy(startFileName, value, STARTFILE_MAX - 1);
      startFileName[STARTFILE_MAX - 1] = '\0';
      found = 1;
    }
    if(!strcmp(name, "startDrop") || !strcmp(name, "StartDrop")) {
      startDrop = atoi(value);
      found = 1;
    }
    if(found)
      continue;
