/*   anneal_opt.c
**
**   Optimized simulated annealing with xorshift64* PRNG.
**   Uses faster random number generation for inner loops.
**
*/
...
/* Use standard PRNG - custom PRNG causes issues on Cygwin */
#define fast_rnd(n) rnd(n)
#define fast_random01() random01()
#define fast_seed(s) /* no-op */
...
static rankType randomNeighborMask(maskType currMask, maskType *outMask)
{
  ...
  removeBit = selectNthBit(currMask, fast_rnd(k));
  addBit = selectNthBit(comp, fast_rnd(v - k));
...
costType computeNeighbor(void)
{
  ...
  /* Merge-diff loop with cached sentinel */
  for(;;) {
    if(*currPtr == *nextPtr) {
...
void acceptNeighbor(void)
{
  ...
  if(onTheFly) {
    ...
    /* Fused loop for better instruction-level parallelism */
